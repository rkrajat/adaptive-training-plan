...
- [ ] 1. Set up backend infrastructure and Strava OAuth configuration
  - [ ] 1.1 Create Strava application on Strava Developer Portal and obtain client ID/secret
  - [ ] 1.2 Add environment variables to apps/api/.env (STRAVA_CLIENT_ID, STRAVA_CLIENT_SECRET, STRAVA_REDIRECT_URI, JWT_SECRET, FRONTEND_URL)
  - [ ] 1.3 Install dependencies: jsonwebtoken in apps/api
  - [ ] 1.4 Configure node-strava-v3 client in apps/api/src/config/strava.ts
  - [ ] 1.5 Update CORS configuration in apps/api/src/index.ts to allow credentials
  - [ ] 1.6 Verify configuration loads correctly on server start

- [ ] 2. Create User model and database schema
  - [ ] 2.1 Create apps/api/src/models/User.ts with Mongoose schema (stravaId, firstName, lastName, profilePhoto)
  - [ ] 2.2 Add unique index on stravaId field
  - [ ] 2.3 Add timestamps option for createdAt/updatedAt
  - [ ] 2.4 Connect to MongoDB in apps/api/src/index.ts
  - [ ] 2.5 Test database connection and User model creation

- [ ] 3. Implement JWT authentication middleware
  - [ ] 3.1 Create apps/api/src/middleware/auth.ts with authenticateJWT middleware
  - [ ] 3.2 Implement JWT token verification using jsonwebtoken
  - [ ] 3.3 Extract userId, stravaId, and stravaAccessToken from token payload
  - [ ] 3.4 Handle missing, invalid, and expired tokens with 401 responses
  - [ ] 3.5 Attach user payload to req.user for downstream use

- [ ] 4. Implement Strava OAuth endpoints
  - [ ] 4.1 Create apps/api/src/routes/auth.ts router
  - [ ] 4.2 Implement GET /api/auth/strava endpoint to redirect to Strava authorization
  - [ ] 4.3 Implement GET /api/auth/strava/callback to handle OAuth callback
  - [ ] 4.4 Exchange authorization code for access token using node-strava-v3
  - [ ] 4.5 Fetch athlete profile from Strava API
  - [ ] 4.6 Create or update User in MongoDB using stravaId
  - [ ] 4.7 Generate JWT token with user data and Strava access token
  - [ ] 4.8 Redirect to frontend with JWT token in query parameter

- [ ] 5. Implement authenticated API endpoints
  - [ ] 5.1 Implement GET /api/auth/me endpoint with JWT middleware
  - [ ] 5.2 Return user profile data from MongoDB
  - [ ] 5.3 Implement GET /api/activities endpoint with JWT middleware
  - [ ] 5.4 Fetch activities from Strava API using access token from JWT
  - [ ] 5.5 Filter activities to last 30 days
  - [ ] 5.6 Return formatted activity data (id, name, distance, moving_time, type, start_date, average_heartrate)
  - [ ] 5.7 Handle Strava API errors and return appropriate status codes

- [ ] 6. Create shared TypeScript types package
  - [ ] 6.1 Create packages/types/src/strava.ts with Activity interface
  - [ ] 6.2 Add User interface for user profile data
  - [ ] 6.3 Export all types from packages/types/src/index.ts
  - [ ] 6.4 Configure package.json with proper exports
  - [ ] 6.5 Build types package with TypeScript
