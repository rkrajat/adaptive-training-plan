...
import { stravaConfig, strava } from '../config/strava';
...
router.get('/strava', (_req: Request, res: Response) => {
  const authUrl = `https://www.strava.com/oauth/authorize?client_id=${stravaConfig.client_id}&response_type=code&redirect_uri=${stravaConfig.redirect_uri}&approval_prompt=force&scope=read,activity:read_all`;
...
router.get('/strava/callback', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const tokenResponse = await (strava as any).oauth.getToken(code);
